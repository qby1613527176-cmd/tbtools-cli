...
# ---- 已知坑位提示 ----
PITFALL_HINTS = {
    "hclust": "⚠️ 输入必须是三列距离文件 GeneA\\tGeneB\\tdist（不是表达矩阵！）",
    "barplot": "⚠️ termCol/pvalCol 用列名（如 Term/Pvalue），不是列索引数字",
    "cubeheatmap": "⚠️ group 文件第一行会被当数据——喂前先去表头",
    "admixture": "⚠️ 第一个参数是 qFiles.lst（每行一个 Q 矩阵文件路径），不是 Q 矩阵内容",
    "dotplot": "⚠️ --chrLayout 传文件路径（内容: Genome: Chr1 Chr2...），不是内联字符串",
    "microsyn": "⚠️ 必须指定 --chr1/--start1/--end1 和 --chr2/--start2/--end2；染色体名须数字",
    "dualsyn": "⚠️ 简化 GFF 染色体名必须数字（parseInt）；需显式 --chr1/--chr2",
    "msy": "⚠️ 简化 GFF 染色体名必须数字；基因名在第 2 列；坐标列不连 -",
    "multisyn": "⚠️ 染色体名必须数字；gxf.lst 路径不能硬编码",
    "pafviz": "⚠️ PAF 文件必须 13 列（不足会 [12] 越界）",
    "pafref": "⚠️ PAF 必须含 cg:Z: CIGAR 标签（minimap2 -c --cs 输出自带）",
    "peaktss": "⚠️ 签名 <gxf> <macs2_peak.xls> <out>——gxf 是必给第 1 参",
    "peakanno": "⚠️ peak 用 MACS2 格式；坐标须百万级 bp（小坐标触发 bin 边界 bug）",
    "supercircos": "⚠️ 配置文件 [chrLen] 后跟文件路径，非内联数据；[link]/[gene]/[track] 同理",
    "gel": "⚠️ LaneLabels 逗号分隔（第一个给 marker）；MarkerRange 降序；FragmentRangeArr 分号泳道/逗号片段",
    "motif": "⚠️ 需真 meme.xml + 序列 ID 匹配的 ID 列表（grep -oP 'name=\"[^\"]+\"' meme.xml 提取）",
    "colorscheme": "⚠️ refColIndex 是 1-based（传 0 会 IndexOutOfBounds）",
    "plotrna": "⚠️ 必须带 --directPDF 否则弹窗；只支持 PDF",
    "pep2codon": "⚠️ 参数顺序 <cds.fa> <pep.aln.fa> <out>（先 CDS 后蛋白比对）",
    "goParse": "⚠️ 产物写到输入文件同目录（<输入名>.TBtools.Parsed.*），无独立输出参数",
    "mcscanx": "⚠️ gff 简化格式 chr\\tgene\\tstart\\tend；blast 用 tab6；classify 须同时给 collinearityFile+geneTypeFile",
    "efpHeat": "⚠️ TGA 底图必须 TrueColor(type2)；需 fake DatatypeConverter",
    "multiEfp": "⚠️ TGA 底图必须 TrueColor(type2)；需 fake DatatypeConverter",
    "annocompare": "⚠️ 输入两个 GFF3 + 输出目录；生成 change_summary.csv + figures/*",
    "onesteptree": "⚠️ --bbTime ≥1000；序列须 ≥4 条唯一（太相似会被合并报错）",
    "nwAlign": "⚠️ 输入文件每行一条序列，无 FASTA 头（传 FASTA 会把 >s1 当序列）",
    "treeRooting": "⚠️ Newick 树必须带枝长（裸 Newick 报 Corrupt NEWICK format）",
    "distance": "⚠️ 方法名小写 euclidean/pearson/pearsonDist；结果输出到 stdout（非文件）",
    "markertools": "⚠️ 结果输出到 stderr（非 stdout！）；$(...) 需 2>&1 捕获",
    "simplehmmscan": "⚠️ 需 Pfam-A.hmm 数据库；idList 每行一个 Pfam NAME（如 GRAS）",
}
