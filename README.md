...
> **174 个绘图/分析命令 + 188 个 RPC 数据工具 + 82 个命令行工具 + 102 个 Java 桥 + 任意引擎反射**，全部实测出图。（数字以 `tbtools version` 实时统计为准）
> 2026-08-31 达成 123 引擎里程碑（含 dualsyn 旧框架保存破解 + eFP 热图/全管线 miRNA/双向 BLAST 等），102 个 Java 桥，174 命令。10 批回归 162/162 PASS。
...
| 文档 | 内容 |
|:-----|:-----|
| [`docs/COMMAND_REFERENCE.md`](docs/COMMAND_REFERENCE.md) | **命令参考手册**：174 个命令（用法表+详细注释）+ 82 个 CLI 工具（18 类功能分组）+ 102 个桥 Javadoc（输入格式权威来源）+ 32 条实测坑位 + engine 反射 + RPC 指引 |
...
├── bridges/               # 102 Java bridge sources
