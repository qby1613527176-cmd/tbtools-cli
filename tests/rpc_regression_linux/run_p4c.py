...
for i, m in enumerate([x for x in methods if x not in SKIP], 1):
    ...
    # 金丝雀：检测输入文件是否被截断
    try:
        sz = os.path.getsize("out/pin10.fa")
        if sz < 1000:
            w(f"\n🚨 **金丝雀报警：{m} 执行后 out/pin10.fa 只剩 {sz} 字节——该方法截断了输入文件！**\n")
            print(f"CULPRIT {m} size={sz}")
            # 立即恢复以继续测试
            io.open("out/pin10.fa", "w").write(io.open("out/pin10.fa.bak").read())
